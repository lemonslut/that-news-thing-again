...
RSpec.describe StoryClusterer do
  # Helper to create an article with subjects
  def create_article_with_subjects(subjects:, published_at: Time.current)
    ...
    subjects.each do |concept|
      article.article_subjects.create!(concept: concept)
    end
    article
  end
  ...
  describe "#call" do
    ...
    it "creates a story from two articles with 50%+ subject overlap" do
      article1 = create_article_with_subjects(
        subjects: [person1, org1],
        published_at: 1.hour.ago
      )
      article2 = create_article_with_subjects(
        subjects: [person1, org2],
    ...
    it "does not cluster articles below 50% overlap" do
      article1 = create_article_with_subjects(
        subjects: [person1, org1, loc1],
        published_at: 1.hour.ago
      )
      article2 = create_article_with_subjects(
        subjects: [person2, org2], # no overlap
        published_at: Time.current
      )
    ...
    it "adds article to existing story with matching subjects" do
      article1 = create_article_with_subjects(
        subjects: [person1, org1],
      ...
      # New article with matching subjects
      article3 = create_article_with_subjects(
        subjects: [person1, org1, loc1],
        published_at: Time.current
      )
    ...
    it "ignores articles older than 7 days when looking for matches" do
      old_article = create_article_with_subjects(
        subjects: [person1, org1],
        published_at: 8.days.ago
      )
      recent_article = create_article_with_subjects(
        subjects: [person1, org1],
        published_at: Time.current
      )
    ...
    it "returns false for articles without subjects" do
      article = Article.create!(
        source_name: "Test",
        title: "No subjects",
        url: "https://example.com/no-subjects",
        published_at: Time.current
      )
    ...
    it "returns false if article already has a story" do
      ...
      article = create_article_with_subjects(subjects: [person1, org1], published_at: Time.current)
    ...
    it "returns true when article is clustered" do
      article1 = create_article_with_subjects(subjects: [person1, org1], published_at: 1.hour.ago)
      article2 = create_article_with_subjects(subjects: [person1, org1], published_at: Time.current)
  ...
  describe "overlap calculation" do
    let!(:c1) { Concept.create!(uri: "llm://person/c1", concept_type: "person", label: "C1") }
    let!(:c2) { Concept.create!(uri: "llm://person/c2", concept_type: "person", label: "C2") }
    let!(:c3) { Concept.create!(uri: "llm://person/c3", concept_type: "person", label: "C3") }
    let!(:c4) { Concept.create!(uri: "llm://person/c4", concept_type: "person", label: "C4") }

    it "clusters at exactly 50% overlap" do
      # Article with 2 subjects, one shared = 50%
      article1 = create_article_with_subjects(subjects: [c1, c2], published_at: 1.hour.ago)
      article2 = create_article_with_subjects(subjects: [c1, c3], published_at: Time.current)
    ...
    it "uses smaller set for ratio calculation" do
      # Article1 has 4 subjects, Article2 has 2
      # 2 shared = 100% of smaller set
      article1 = create_article_with_subjects(subjects: [c1, c2, c3, c4], published_at: 1.hour.ago)
      article2 = create_article_with_subjects(subjects: [c1, c2], published_at: Time.current)
